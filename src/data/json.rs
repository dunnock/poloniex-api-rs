...
impl Expect<f32> for JsonValue {
  type Error = PoloError;
  fn expect(&self, msg: &str) -> Result<f32, Self::Error> {
    let err = || PoloError::wrong_data(format!("{}: expected float got {}", msg, self));
    if self.is_string() {
      self.as_str().ok_or_else(err)?.parse::<f32>().map_err(PoloError::from)
    } else {
      self.as_f32().ok_or_else(err)
    }
  }
}
...
impl Expect<HashMap<String, f32>> for JsonValue {
  type Error = PoloError;
  fn expect(&self, msg: &str) -> Result<HashMap<String, f32>, Self::Error> {
    if !self.is_object() {
      return Err(PoloError::wrong_data(format!("{}: expected object", msg)));
    }
    let mut hash = HashMap::new();
    for (rate, amount) in self.entries() {
      let amount: f32 = amount.expect("expected float amount")?;
